...
void open_file(int argc,char *argv[])
{
    ...
    { 
        ...
        else { printf("0"); exit(2); }
		
        cn=b+1;    
        function(M);                                         //call function that creates the number N
    }
}
...
{ 
    while(!(i==j) && !(i==j-1))
    {
         ...
         if((((s[i]*10+s[i+1])==(s[j]+10)) || ((s[i]*10+s[i+1]-1)==(s[j]+10)) || ((s[i]*10+s[i+1]-1)==s[j])) && flag2==0)   flag=0;
         
         else if (((s[i]==s[j]) || ((s[i] - 1)==s[j])) && flag2==1)   flag=1;    
